 ...
 */
export const getDisplayPath = (absolutePath, cwd) => {
 ...
 */
export const reportResults = (cwd, packResult, config, options = {}) => {
    logger.log('');
...
export const reportSummary = (cwd, packResult, config, options = {}) => {
    var _a, _b;
    ...
    if (config.skillGenerate !== undefined && options.skillDir) {
        ...
        if (packResult.outputFiles && packResult.outputFiles.length > 0) {
            ...
            logger.log(`${pc.white('       Output:')} ${pc.white(firstDisplayPath)} ${pc.dim('…')} ${pc.white(lastDisplayPath)} ${pc.dim(`(${packResult.outputFiles.length} parts)`)}`);
        }
        else {
